...
namespace CSharpSynth.Wave {
  public class WaveHelper {
    ...
    public static float[,] DownSample(int factor, float[,] data) {//skipping samples
      if (factor == 1) {
        return data;
      }
    ...
    public static float[,] UpSample(int factor, float[,] data) {//zero padding
      if (factor == 1) {
        return data;
      }
        ...
        for (int i = 0; i < channels; i++) {
          switch (bytesPerSample) {
            case 1:
              ...
              break;
            case 2:
              samples[i, x] = BitConverter.ToInt16(datachk.sampled_data, (x * 2 * channels) + (i * 2)) / (float)short.MaxValue;
              break;
            case 3:
              samples[i, x] = ToInt24(datachk.sampled_data, (x * 3 * channels) + (i * 3)) / (float)8388607;
              break;
            case 4:
              samples[i, x] = BitConverter.ToInt32(datachk.sampled_data, (x * 4 * channels) + (i * 4)) / (float)int.MaxValue;
              break;
          }
        }
      }
        ...
        for (int c = 0; c < channels; c++) {
          ...
          switch (increment) {
            case 1:
              ...
              break;
            case 2:
              buffer = BitConverter.GetBytes((short)(sampledata[c, x] * short.MaxValue));
              rawdata[counter] = buffer[0];
              rawdata[counter + 1] = buffer[1];
              break;
            case 3:
              buffer = Int24toBytes((int)(sampledata[c, x] * 8388607));
              rawdata[counter] = buffer[0];
              rawdata[counter + 1] = buffer[1];
              rawdata[counter + 2] = buffer[2];
              break;
            case 4:
              buffer = BitConverter.GetBytes((int)(sampledata[c, x] * int.MaxValue));
              rawdata[counter] = buffer[0];
              rawdata[counter + 1] = buffer[1];
              rawdata[counter + 2] = buffer[2];
    ...
    public static int ToInt24(byte[] buffer, int index) {
    ...
    public static byte[] Int24toBytes(int value) {
