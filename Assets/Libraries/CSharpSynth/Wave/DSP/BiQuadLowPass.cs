...
namespace CSharpSynth.Wave.DSP {
	public class BiQuadLowPass
    {
        ...
        public BiQuadLowPass(float sampleRate, int channels, float cornerfrequency)
        {
			...
			this.temps = new float[channels, 2];
			this.ResetFilter(sampleRate, cornerfrequency, 1.0f);
        }
        public void ResetFilter(float sampleRate, float cornerFrequency, float Q)
        {
            ...
            Array.Clear(this.temps, 0, this.channels * this.temps.GetLength(1));
        }
        public void ApplyFilter(float[,] inputBuffer)
        {
            ...
            for (int x = 0; x < this.channels; x++)
            {
                ...
                {
                    inputBuffer[x, x2] = (this.b0 / this.a0) * inputBuffer[x, x2] + (this.b1 / this.a0) * this.temps[x, 0] + (this.b2 / this.a0) * this.temps[x, 1] - (this.a1 / this.a0) * this.temps[x, 0] - (this.a2 / this.a0) * this.temps[x, 1];
					this.temps[x, 1] = this.temps[x, 0];
					this.temps[x, 0] = inputBuffer[x, x2];
                }
            }
        }
        //--Static
